...
for ((i=0; i<process_count; i++)); do
    ...
    for ((j=0; j<block_count; j++)); do
        if [ ${block_size[$j]} -ge ${process_size[$i]} ] && [ ${block_allocated[$j]} -eq 0 ]; then
            if [ $worst_fit_index -eq -1 ] || [ ${block_size[$worst_fit_index]} -lt ${block_size[$j]} ]; then
                worst_fit_index=$j  # Update worst fit index
            fi
        fi
    done
    if [ $worst_fit_index -ne -1 ]; then
        block_allocated[$worst_fit_index]=1  # Mark block as allocated
        process_allocated[$i]=$((worst_fit_index+1))  # Store allocated block index
    else
